...
class FichierClient(object):
    ...
    def __init__(self, APIkey=None, be_nice=False):
        if APIkey:
            self.auth = {'Authorization': f'Bearer {APIkey}',
    ...
    def _raise_unauthorized(self):
        raise NotAuthorized(
            'You are using a very limited version of the API, and the feature you are trying to use requires an APIkey.')
    ...
    def _APIcall(self, url, json=None, method='POST'):
        if method == 'POST':
            ...
            raise FichierSyntaxError(
                f'Method {method} not available/implemented')
        if r.ok:
            try:
                o = r.json()
            ...
            if 'status' in o:
                if o['status'] == 'OK':
                    ...
                    raise FichierResponseNotOk(
                        f'Response from 1fichier: {message!r}')
            else:
            ...
            raise FichierResponseNotOk(
                f'HTTP Response code from 1fichier: {r.status_code} {r.reason}')

    def resolve_path(self, path):
        print(f'Resolving {path!r}...')
        if not path.startswith('/'):
            raise FichierSyntaxError(
                'Paths must start from root, aka start with a forward slash ("/")')
        folder_paths = path.split('/')
        del folder_paths[0]
        folder = self.get_folder()
        for idx, folder_path in enumerate(folder_paths):
            if idx+1 < len(folder_paths):
                folder = folder.subfolders.get_subfolder(
                    folder_path, only_subfolders=True)
            else:
    ...
    def remote_upload_info(self, id=None, only_data=False):
        ...
        if id is None:
            raise InsufficientInfoError(
                'We need an id of an exisiting remote upload via "id" param')
        o = self._APIcall(
            'https://api.1fichier.com/v1/remote/info.cgi', json={'id': id})
        if o is not None:
            if only_data:
                return o['result']
            else:
    ...
    def remote_upload_create(self, urls=None, headers=None):
        ...
        if urls is None:
            raise InsufficientInfoError(
                'We need a list of urls to upload via "urls" param')

        upload_info = {'urls': urls}
        if headers:
            upload_info.update({'headers': headers})
        o = self._APIcall(
            'https://api.1fichier.com/v1/remote/request.cgi', json=upload_info)
        if o is not None:
            # ~ if only_data:
            # ~ return o['result']
            # ~ else:
    ...
    def _get_folders(self, id=0):
        ...
        params = {'folder_id': id}
        o = self._APIcall(
            'https://api.1fichier.com/v1/folder/ls.cgi', json=params)
        return o
    ...
    def _get_files(self, id=0):
        ...
        params = {'folder_id': id}
        o = self._APIcall(
            'https://api.1fichier.com/v1/file/ls.cgi', json=params)
    ...
    def get_download_link(self, url, inline=False, cdn=False, restrict_ip=False, passw=None,
        ...
        if restrict_ip:
            if not cdn:
                if self.be_nice:
                    ...
                    raise FichierSyntaxError(
                        'Restricting IPs is only for CDN links')
        params = {
            'url': url,
            'inline': int(inline),
            'cdn': int(cdn),
        ...
        if folder_id is not None:
            if filename is None:
                raise FichierSyntaxError(
                    'Also need a filename to go along with that')
            params.update({'folder_id': folder_id, 'filename': filename})
            if folder_id == 0:
                if sharing_user is None:
                    raise FichierSyntaxError(
                        'sharing_user not specified but required')
                params.update({'sharing_user': sharing_user})
        #~ print(params)
        o = self._APIcall(
            'https://api.1fichier.com/v1/download/get_token.cgi', json=params)
        return o['url']
    ...
    def upload_file(self, file_path):
        ...
        if self.authed is True:
            r = s.post(up_u, files=multiple_files,
                       headers=self.auth_nc, allow_redirects=False)
        else:
    ...
    def get_file_info(self, url, passw=None, folder_id=None, filename=None, sharing_user=None):
        ...
        o = self._APIcall(
            'https://api.1fichier.com/v1/file/info.cgi', json=params)
        return o
    ...
    def virus_scan(self, url):
        ...
        o = self._APIcall(
            'https://api.1fichier.com/v1/file/scan.cgi', json=params)
        return o
    ...
    def remove_file(self, urls, codes=None):
        ...
        if codes:
            try:
                ...
                raise FichierSyntaxError(
                    'If codes specified, it must be at least the length of the url list')
        else:
            params = {
        ...
        o = self._APIcall(
            'https://api.1fichier.com/v1/file/rm.cgi', json=params)
    ...
    def move_file(self, urls, destination_folder=None, destination_user='', rename=''):
        ...
        if not destination_folder and destination_user:
            raise FichierSyntaxError(
                'If destination_folder unspecified or 0, destination_user must be specified')
        ...
        if destination_folder:
            ...
            params.update({'destination_folder_id': 0,
        ...
        o = self._APIcall(
            'https://api.1fichier.com/v1/file/mv.cgi', json=params)
    ...
    def copy_file(self, urls, destination_folder=None, destination_user='', rename='', passw=''):
        ...
        o = self._APIcall(
            'https://api.1fichier.com/v1/file/cp.cgi', json=params)
